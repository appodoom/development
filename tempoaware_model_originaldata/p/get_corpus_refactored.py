...
# Configurations
MEL_JSON_PATH = Path("fundamentals_spectrograms.json")
AUDIO_FOLDER = Path("./audios")
CORPUS_OUTPUT_FOLDER = Path("corpus-originaldata")
...
def get_onsets(y, sr):
    ...
    return librosa.frames_to_samples(onsets_avg)
...
def get_corpus(fundamentals_path, data_path):
    ...
    fundamentals = load_mel_from_json(fundamentals_path)
    ...
    for interval in intervals:
        ...
        best_note = min(
            note_durations.keys(),
            key=lambda note: abs(note_durations[note] - hit_duration)
        )
        classified_hits.append(f"{best_hit}_{best_note}")
    
    return classified_hits

def main():
    CORPUS_OUTPUT_FOLDER.mkdir(parents=True, exist_ok=True)
    ...
    for filename in os.listdir(AUDIO_FOLDER):
        if filename.endswith(".wav"):
            ...
            try:
                hits = get_corpus(MEL_JSON_PATH, filepath)
                output_path = CORPUS_OUTPUT_FOLDER / filename.replace('.wav', '.txt')
    ...
    print(f"\n🎉 All files processed. Corpora saved to {CORPUS_OUTPUT_FOLDER}")
