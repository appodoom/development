...
# CKPT_PATH = "./pairs_model/pairs_model.pt"
...
#         )
#     mids = [onset_frames[i] for i in range(len(onset_frames))]
...
class SmallAudioCNN(nn.Module):
    """
    2-channel CNN: input shape (B, 2, n_mels, T)
    Channel 0: previous hit
    Channel 1: current hit
    """

    def __init__(self, n_classes):
        super().__init__()
        self.features = nn.Sequential(
            nn.Conv2d(2, 16, kernel_size=3, padding=1),  # <-- 2 channels
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2),
            nn.Conv2d(16, 32, kernel_size=3, padding=1),
            nn.ReLU(),
...
def compute_mel_array(y, sr, target_frames: int = TARGET_FRAMES) -> np.ndarray:
    """
    Compute *one* log-mel spectrogram for one hit.
...
def build_pair_tensor(prev_mel: np.ndarray, curr_mel: np.ndarray) -> torch.Tensor:
    """
    prev_mel, curr_mel: (n_mels, T)
    Stack -> (2, n_mels, T) and add batch dimension -> (1, 2, n_mels, T)
    """
    pair = np.stack([prev_mel, curr_mel], axis=0)  # (2, n_mels, T)
    x = torch.from_numpy(pair).unsqueeze(0)  # (1, 2, n_mels, T)
    return x.to(device)


@torch.no_grad()
def predict_hit(model: nn.Module, prev_y, curr_y, sr):
    """
    prev_y, curr_y: waveforms (1D numpy arrays) for previous and current hits.
    Returns label of the CURRENT hit + probs.
    """
    prev_mel = compute_mel_array(prev_y, sr)
    curr_mel = compute_mel_array(curr_y, sr)
    xb = build_pair_tensor(prev_mel, curr_mel)  # (1, 2, n_mels, T)
    logits = model(xb)
    probs = torch.softmax(logits, dim=1).cpu().numpy()[0]
    pred_idx = int(np.argmax(probs))
    return CLASSES[pred_idx], probs
...
def plot_mel(
    ...
    Plot and save a Mel spectrogram.
    """
    out_path = f"{out_path}_{index}.png"
...
def get_corpus(fundamentals_path, file_path, model_pred, log_mel):
    """
    If model_pred=True -> use CNN pairs model.
    If model_pred=False -> use sliding cross-correlation against fundamentals.
    """
    y, sr = load_file(file_path)
    # resample to model's training rate
    y = librosa.resample(y, orig_sr=sr, target_sr=SAMPLE_RATE)
    sr = SAMPLE_RATE

    # prepend 1 second of silence (for onset detection stability etc.)
    y = np.concatenate([np.zeros(sr), y])
    ...
    if model_pred:
        # ===== CNN model branch (pairs) =====
        prev_segment = None

        for i, (interval, interval_dur) in enumerate(
            zip(intervals, intervals_duration)
        ):
            segment = y[interval[0] : interval[1]]  # current hit waveform

            if prev_segment is None:
                # For the very first hit, re-use the same segment as "previous"
                prev_segment = segment

            # optional mel for debug/plot
            mel = librosa.feature.melspectrogram(y=segment, sr=sr)
            if log_mel:
                mel = librosa.power_to_db(mel, ref=np.max, top_db=80)
                # plot_mel(i, mel, sr=sr, hop_length=HOP_LENGTH)

            best_hit, _ = predict_hit(
                model=MODEL,
                prev_y=prev_segment,
                curr_y=segment,
                sr=sr,
            ...
            hit_duration = (interval_dur[1] - interval_dur[0]) / sr  # seconds
...
# )
# print("tempo:", tempo)
# print("classified hits (CNN):", classified_hits_with_model_pred)
