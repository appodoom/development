...
def main():
    atom_pattern = re.compile(r'[A-Za-z]+_[0-9]+(?:\.[0-9]+)?_[0-9]+')

    # Load vocabulary merges
    vocab_path = Path('vocab_music_sheet_5000.txt')
    ...
    tokens_dir = Path('demo_with_amp/tokens_with_amp')
    tokens_dir.mkdir(exist_ok=True)

    # Process each corpus file
    corpus_dir = Path('demo_with_amp/corpus_with_amp')
    for corpus_file in sorted(corpus_dir.glob('corpus*.txt')):
