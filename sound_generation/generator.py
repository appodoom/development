...
in_maxsubd = 8 # smallest note is an 8th of a beat
in_num_cycles = 3 #cycles

def generate_audio(bpm, skeleton, maxsubd, num_cycles, out_path, sr=48000):

    beat_length_in_samples = int((60/bpm)*sr)
    skeleton_length = len(skeleton)
    num_of_beats_in_audio = num_cycles*skeleton_length

    length_in_samples = int(sum([x[0] * beat_length_in_samples for x in skeleton]) * num_cycles)
...
generate_audio(in_bpm, in_skeleton, in_maxsubd, in_num_cycles, "test.wav")