...
vocab_path = os.path.join(root, "vocab_music_sheet_2000.txt")
tokens_dir = os.path.join(root, "tokens_music_sheet")
...
print(f"Total batches before training: {total_batches}")
    ...
    for loader in train_loaders:
        for xb, yb in loader:
            ...
            if total_batches % 10 == 0:
                print(
                    f"Epoch {ep} batch {total_batches} loss {total_loss / total_batches:.4f}",
                    flush=True,
                )
...
torch.save(
    model.state_dict(),
    os.path.join(save_dir, "music_sheet_transformer_weights_5_epoch.pth"),
)
