...
def main() -> None:
    ...
    if not vocab_path.exists():
        raise SystemExit(f"Missing {vocab_path.resolve()} — run vocab builder first.")
    ...
    if not (DATA_DIR.exists() and DATA_DIR.is_dir()):
        raise SystemExit(f"DATA_DIR {DATA_DIR.resolve()} does not exist.")
